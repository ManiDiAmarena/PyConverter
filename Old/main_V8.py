...
# --- NUOVO: Per Velocità, rispetto a Metri al Secondo (m/s) ---
FATTORI_VELOCITA_A_METRI_PER_SECONDO = {
    "Metri al secondo (m/s)": 1.0,
    "Chilometri all'ora (km/h)": 1000.0 / 3600.0,  # 1 km/h = 0.277... m/s
    "Miglia all'ora (mph)": 1609.34 / 3600.0,    # 1 mph = 0.44704 m/s
    "Nodi (kn)": 1852.0 / 3600.0                # 1 nodo = 0.514444... m/s
}
...
def esegui_conversione():
    ...
    if not valore_input_str:
        ...
        try:
            ...
            elif categoria == "Velocità": # Nuova logica per Velocità
...
# La larghezza potrebbe aver bisogno di un ulteriore aggiustamento per i nomi delle unità di velocità
finestra.geometry("520x300") # Aumentata leggermente per la nuova categoria e nomi unità
...
# Aumentata la larghezza del combobox categoria
combo_categoria = ttk.Combobox(frame_input, values=categorie_disponibili, width=26, state="readonly")
...
entry_valore = ttk.Entry(frame_input, width=32) # Aumentata la larghezza
...
combo_unita_partenza = ttk.Combobox(frame_input, width=26, state="readonly") # Aumentata la larghezza
...
combo_unita_destinazione = ttk.Combobox(frame_input, width=26, state="readonly") # Aumentata la larghezza
